...
spe_taxa <- "~/Desktop/Microbiome/Kingdom_effect/working_tables/PV4_OTU_table_merged_Endo3055_Epi412_conv_L6.txt"
spe_tab <- read.csv(spe_taxa, sep = "\t", dec = ".", header=T, row.names= 1, skip =1) 
map <- read.csv("~/Desktop/Microbiome/Kingdom_effect/Mapfile_rare_taxa.txt", sep = "\t", dec =".", header = T, row.names=NULL)

divisore <- match(colnames(spe_tab),map$X.SampleID)
...
kquery <- args[1]
div_choice <- args[2] #allowed: [B] -> Bacteria, [F] -> Fungi, [O] -> Oomycetes
tr <- 10 #args[2]
if(div_choice == "B"){
...
dir <- paste("./",kquery,"_rare_taxa_on_",name,"_thresh_",tr,"perc","/", sep= "")
dir.create(dir)
...
if(kquery == "general"){
  ...
  rare <- grepl(kquery,rownames(spe_tab))
  rare_index <- which(rare == "TRUE")
  rare_sub <- spe_tab[rare_index,]
}

rare_sum <- rowSums(rare_sub)
val <- unname(rare_sum)
nam <- names(rare_sum)
q <- quantile(val,probs=seq(0,1,0.05))
tre <- paste(tr,"%", sep ="")
thresh <- unname(q[which(names(q)==tre)])
rare_taxa_indx <- which(val <= thresh, arr.ind = TRUE) 
rare_taxa <- rare_sum[rare_taxa_indx]
new_query <- names(rare_taxa)
rare_df <- spe_tab[match(new_query,rownames(spe_tab)),]

cue_sum <- colSums(rare_df)  # sum all the abundances per sample 
...
cue_pos
cue_neg
w_cue <- rare_df[,cue_pos]  #from the cuero_subset, picks up only the samples with at least one otus assigned to cuero
wo_cue <- rare_df[,cue_neg]  #from the cuero_subset, picks up only the samples in which we have no otus assigned to cuero
...
wcue_df <- data.frame(ID= w_cuein, Cue_Status = paste(kquery,"_rare_taxa_positive", sep = ""), Cue_Abundance = colSums(w_cue), row.names=1)
wocue_df <- data.frame(ID= wo_cuein, Cue_Status= paste(kquery,"_rare_taxa_negative", sep = ""),Cue_Abundance = colSums(wo_cue), row.names=1)
